...
By simply including `LEProc.exe`, the configuration `le.config`, and the dlls `LoaderDll.dll` and `LocaleEmulator.dll`,
...
* `Parameter`: The relative path from the directory of `LEProc.exe` to the target executable you want to run.
...
can include this launcher in your patch and tell users to run `LEProc.exe` to run the translated patched game, allowing
...
Before you zip this up, you add `LEProc.exe`, `le.config`, `LoaderDll.dll`, and `LocaleEmulator.dll` into the root folder
alongside `game.exe`.

To ensure it runs in the Japanese locale, you edit `le.config` and make the `Location` value to be `ja-JP` and make the
`Timezone` value to be `Tokyo Standard Time`. To ensure `LEProc.exe` launches the game you also make the `Parameter` value
to be `game.exe`, as that is the relative path to that executable from `LEProc.exe`'s directory. You set the other parameters
as appropriate.

If you want, you can also rename `LEProc.exe` to whatever is best for you (like `game-en.exe` or something) to make it
...
Now you can just zip that up with the included laucnher in your patch distrubation, and tell users to run `LEProc.exe`
